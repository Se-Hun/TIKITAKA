...
"""
import sys # For Debugging(From sehun)
import asyncio # For async-await(From sehun)
import codecs
import glob
import gc
import torch
...
def process_one_shard(corpus_params, params):
    ...
    logger.info("data.save finished")

    del dataset.examples
    #logger.info("dataset.examples : " + dataset.examples)
    gc.collect()
    #logger.info("dataset.examples free~~")
    del dataset
    gc.collect()
    #logger.info("dataset free~~")
    #logger.info(sub_sub_counter)
...
def build_save_dataset(corpus_type, fields, src_reader, tgt_reader,
    ...
    shard_iter = shard_iterator(srcs, tgts, ids, aligns, existing_shards,
                                existing_fields, corpus_type, opt)
    #logger.info(sys.getsizeof(shard_iter.__self__.bitstring_cache) / 1024 / 1024)
    ...
    for sub_counter in map(func, shard_iter):
    ...
    #with Pool(opt.num_threads) as p:
        ...
        #print(func)
        #print(shard_iter)
        #temp = p.apply_async(func, shard_iter)
        #try:
        #    print(temp.get(timeout=1))
        #except TimeoutError:
        #    print("sorry")

        #temp = [p.apply_async(func, s) for s in shard_iter]
        #temp = [p.apply_async(func, shard_iter)]
        #temp = [p.apply_async(func, (s, y)) for s,y in shard_iter]
        #temp = p.apply_async(func, shard_iter)

        #temp = p.apply_async(func, shard_iter)
        #temp = temp.get(timeout=1)
        #if len(p._cache) > 1e6:
        #    print("waiting please...")

        #for sub_counter in temp:
        for sub_counter in p.map(func, shard_iter):
            #if len(p._cache) > 1e6:
            #    print("Waiting please..")
        #for sub_counter in temp.get():
            #logger.info(sub_counter)
            #sub_counter = sub_counter.get()
            #print(sub_counter)
            #logger.info("sub_counter : " + sub_counter)
            if sub_counter is not None:
                for key, value in sub_counter.items():
                    #print(key)
                    #print(value)
    ...
    if corpus_type == "train":
        ...
        #model = torch.load(vocab_path)
        #model.to(device)
        #logger.info(model)
        logger.info("----------------------------")

    #logger.info("-------------------------------------------")
    return True # For Debugging!!!(from sehun)
...
def preprocess(opt):
    ...
    is_train_dataset_finished = build_save_dataset(
    ...
    #    logger.info("fuck") # For Debugging(From sehun)
